...
from app.services.hermes.feedback import FEEDBACK_FILENAME
...
def test_apply_creates_page_and_log(tmp_path: Path) -> None:
    ...
    feedback = wiki_dir / "LIE-001" / FEEDBACK_FILENAME
    assert feedback.is_file()
    [line] = feedback.read_text(encoding="utf-8").splitlines()
    row = json.loads(line)
...
def test_apply_idempotent_feedback_on_replay(tmp_path: Path) -> None:
    ...
    plan = PatchPlan(
        ...
        summary="leak",
        event_type="manual",
        ops=[PrependLogOp(line="## [2026-04-25] manual | replay")],
    )
